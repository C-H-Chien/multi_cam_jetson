...
class ArducamUtils(object):
    ...
    @staticmethod
    def _detect_jetson_module():
        try:
            jtop_mod = importlib.import_module('jtop')
            jtop_cls = getattr(jtop_mod, 'jtop')
        ...
        try:
            with open('/proc/device-tree/model', 'rb') as f:
                model = f.read().decode('utf-8', errors='ignore').replace('\x00', '').strip()
                if model:
                    return model
        except Exception:
            pass

        env_module = os.environ.get('JETSON_MODULE')
        if env_module:
            return env_module
    ...
    def __init__(self, device_num):
        environment_vars = ArducamUtils._detect_jetson_module()
